...
<mapper namespace="${daoFullType}">
    ...
    <#list fieldAndColumns as fieldAndColumn>
        <result column="${fieldAndColumn.column}" property="${fieldAndColumn.field}"
                jdbcType="${fieldAndColumn.jdbcType}"/>
    </#list>
    </resultMap>
    ...
    <insert id="insertSelective"<#if useGeneratedKeys> useGeneratedKeys="true" keyProperty="${pojoName}.${primaryField}"</#if>>
        ...
        <trim prefix="(" suffix=")" suffixOverrides=",">
        <#list fieldAndColumns as fieldAndColumn>
            <if test="${pojoName}.${fieldAndColumn.field}!=null">${r"#"}
                {${pojoName}.${fieldAndColumn.field}<#if primaryJdbcType??>,jdbcType=${fieldAndColumn.jdbcType}</#if>},
            </if>
        </#list>
        </trim>
    </insert>
...
<#if currentDatabase=="MySql">
    <insert id="insertList">
        ...
        <foreach collection="${pojoName}s" item="${pojoName}" index="index" separator=",">
            (
            <#list fieldAndColumns as fieldAndColumn>
            ${r"#"}{${pojoName}.${fieldAndColumn.field},jdbcType=${fieldAndColumn.jdbcType}
...
<#if currentDatabase=="Oracle">
    <insert id="insertList">
        ...
        <foreach collection="${pojoName}s" item="${pojoName}">
            ...
            <#list fieldAndColumns as fieldAndColumn>
            ${r"#"}{${pojoName}.${fieldAndColumn.field}<#if primaryJdbcType??>,jdbcType=${fieldAndColumn.jdbcType}</#if>
                }<#rt>
                <#lt><#if !fieldAndColumn?is_last>,</#if>
            </#list>
            )
    ...
    <update id="update">
        ...
        <#list fieldAndColumns as fieldAndColumn>
            <#if fieldAndColumn?is_last>
                <if test="${pojoName}.${fieldAndColumn.field} != null"> ${fieldAndColumn.formattedColumn}
                    = ${r"#"}{${pojoName}.${fieldAndColumn.field},jdbcType=${fieldAndColumn.jdbcType}}
                </if>
            <#else>
                <if test="${pojoName}.${fieldAndColumn.field} != null"> ${fieldAndColumn.formattedColumn}
